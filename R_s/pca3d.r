...
.show.axe.titles <- function(r, show.scale, axes.color, components, axe.titles,...) {
      ...
      texts3d(
        c(cexx *r[ 2,1 ], 0, 0),
        c(0, cexx *r[ 2,2 ], 0),
        c(0, 0, cexx *r[ 2,3 ]), col= "black", axe.titles,cex=pars[["cex"]] ) ;
...
## create a 3D biplot
.biplot.3D <- function(biplot.coords, biplot.vars,loading_col, ranges= NULL,...) {
  ...
  arrows3d(coords, col= loading_col, scale= scale*0.1 ,radius=0.1)
  ...
  pars<-list(...)
  texts3d(coords[,1], coords[,2], coords[,3], names, col= "black", cex=pars[["cex"]] );
}
...
.rgl.window.init <- function(bg) {
  ...
  rgl.viewpoint(theta= 45, phi= 30, fov= 60, zoom= 0.6)
  bg3d(bg)
  rgl.pop("lights")
...
pca3d <- function(pca, components=1:3, col=NULL, title=NULL, new=FALSE,
  ...
  # add the biplot
  if(biplot) 
    .biplot.3D(biplot.coords, biplot.vars,loading_col, main.ranges,...)
