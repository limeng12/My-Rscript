...
arrows3d <- function( coords, headlength= 0.035, head= "end", scale= NULL, radius = NULL,col="red", ... ) {
  ...
  for(i in 1:narr)
    segments3d( coords[(i*2-1):(i*2),],col=col[i],... )
  ...
  if( head == "end" | head == "both" ) {
    for( i in 1:narr ) {
      ...
      if(length(col)==1){
        ...
        cone3d( base, tip,  radius,col[i], scale= scale, ... )
      }
