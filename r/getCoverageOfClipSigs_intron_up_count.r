...
start_pos<-as.numeric(sapply(strsplit(exonId,":"),"[",3) );
end_pos<-as.numeric(sapply(strsplit(exonId,":"),"[",5) );

start_neg<-as.numeric(sapply(strsplit(exonId,":"),"[",9) );
end_neg<-as.numeric(sapply(strsplit(exonId,":"),"[",5) );
...
geneClipMatch<-data.frame(genename=c(),clipname=c(),start=c(),end=c(),functions=c(),exonId=c(),start_rbp=c(),end_rbp=c(),chr_rbp=c(),stringsAsFactors = F);
...
ddply(geneClipMatch,.(target),function(x){
  target_name<-x[1,"target"];
  target_name_file<-paste0("../rbp_bind_seq/",target_name,"_intron_up");
...
#b<-dcast(geneClipMatch,gene_name+start+end+functions+exonId~target);
...
e_intron_up<-d[!is.na(d[,"functions"]),];
#e<-[,"exonId"]<-e[,"exonId"]
write.table(e_intron_up,file="gene_clip_target_se_intron_up.tsv",sep="\t");
