...
geneClipMatch<-data.frame(genename=c(),clipname=c(),start=c(),end=c(),functions=c(),exonId=c(),start_rbp=c(),end_rbp=c(),chr_rbp=c(),strand_rbp=c(),stringsAsFactors = F);
...
ddply(geneClipMatch,.(target),function(x){
  ...
  target_name_file<-paste0("../rbp_bind_seq/",target_name,"_intron_down");
